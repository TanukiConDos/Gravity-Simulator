...
run :: proc(n, ticks: int, interval, theta: f32) {
	...
	fmt.printf("n=%d ticks=%d interval=%8.0f theta=%.1f: total=%8.2f ms  avg=%6.2f ms/tick\n", n, ticks, interval, theta, dt, dt / f64(ticks))
}

main :: proc() {
	run(100000, 10, 1.0, 0.5)
	run(100000, 10, 100000.0, 0.5)
	run(100000, 10, 1.0, 0.8)
	run(100000, 10, 100000.0, 0.8)
}
